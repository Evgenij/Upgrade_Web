...
try{
    $sql = 'SELECT project.id_project, project.name, project.descr, project.mark, project.creator,  
    ...
    if($nestedSQL->rowCount() > 0){
        ...
        while ($project = $nestedSQL->fetch(PDO::FETCH_BOTH)) {
            ...
            if($project['creator'] == 1){
                $buttons = '<svg class="change-data-block svg-fill list-hor__item" width="17" height="20" viewBox="0 0 17 20" fill="none" xmlns="http://www.w3.org/2000/svg">
